...
module.exports = {
  config: {
    ...
    countDown: 0,
    role: 0,
    shortDescription: "Cute AI Sim Chatbot (Auto Teach + Typing)",
    longDescription: "Talk & Chat with Emotion — Auto teach enabled with typing effect.",
    category: "fun",
    guide: {
      en: "{p}sim [message]\n{p}sim teach [Question] - [Answer]\n{p}sim list"
    }
  },
  ...
  onStart: async function ({ api, event, args, message, usersData }) {
    ...
    try {
      ...
      if (!responses || responses.length === 0) {
        ...
        await axios.get(`${simsim}/teach?ask=${encodeURIComponent(query)}&ans=${encodeURIComponent("hmm got it. (auto learned)")}&senderName=${encodeURIComponent(senderName)}`);
        return message.reply("hmm got it.");
      }
      ...
      console.error("❌ Sim main error:", err);
      message.reply(`Error in Sim command: ${err.message}`);
    }
  },
  ...
  onReply: async function ({ api, event, Reply, message, usersData }) {
    ...
    try {
      ...
      if (!responses || responses.length === 0) {
        ...
        await axios.get(`${simsim}/teach?ask=${encodeURIComponent(replyText)}&ans=${encodeURIComponent("hmm got it. (auto learned)")}&senderName=${encodeURIComponent(senderName)}`);
        return message.reply("hmm got it.");
      }
      ...
      console.error("❌ Sim reply error:", err);
      message.reply(`Error in Sim reply: ${err.message}`);
    }
  },
  ...
  onChat: async function ({ api, event, message, usersData }) {
    ...
    try {
      const simpleTriggers = ["sim", "bot", "Eina", "Sim", "Bot"];
      if (simpleTriggers.includes(raw)) {
        await sendTyping();
        const replies = ["Why are you calling me?", "Bakit mo'ko tinatawag kupal?", "Ang ingay mo", "Yes sir?", "Bakit nanaman?", "Don't call me ever again."];
      ...
      // যদি “baby [text]” হয়
      const prefixes = ["Sim", "bot ", "Bot ", "sim"];
      ...
      console.error("❌ Sim onChat error:", err);
    }
  }
};
